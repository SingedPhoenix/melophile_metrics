...
function readStoredSpotifyToken() {
  return readJson<{ access_token?: string; refresh_token?: string; expires_at?: number } | null>(spotifyTokenKey, null);
}

function readActiveSpotifyToken() {
...
async function getValidSpotifyToken(clientId?: string) {
...
async function spotifyFetch(pathOrUrl: string, token: { access_token?: string }, attempt = 0): Promise<any> {
